...
TARGET="$TARGET_DIR/box64-0.4.1.tzst"

AMOD_COMMIT="4ad48931e9aaf77063b71f59f62378521cfa3d95"
AMOD_BLOB_SHA="4b91f41ad7a82b4a6a40fd7a9ea1aa98f4965d6a"
AMOD_URL="https://raw.githubusercontent.com/afeimod/winlator-mod/$AMOD_COMMIT/app/src/main/assets/box64/box64-0.4.1.tzst"
...
if [ -f "$TARGET" ]; then
    ...
    if [ "$ACTUAL" = "$AMOD_BLOB_SHA" ]; then
        echo "AMOD Box64 0.4.1 already materialized and verified"
        exit 0
    fi
fi

echo "Downloading pinned AMOD Box64 0.4.1 from commit $AMOD_COMMIT"
curl --fail --location --retry 3 --retry-delay 2 --output "$TMP" "$AMOD_URL"

ACTUAL=$(git hash-object "$TMP")
if [ "$ACTUAL" != "$AMOD_BLOB_SHA" ]; then
    echo "AMOD Box64 blob verification failed" >&2
...
echo "AMOD Box64 0.4.1 materialized: $TARGET"
